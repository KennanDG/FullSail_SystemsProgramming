...
namespace Helper
{
	...
	void CopyString(const char* source, char*& destination) // Deep copies a string into a char array.
	{
		delete[] destination; // deletes whatever was previously in the destination parameter.

		size_t length = strlen(source) + 1; // Adds 1 to the length of the string to account for the null terminator '\0'
		destination = new char[length]; // Allocates memory in the heap for a new char array.
		strcpy_s(destination, length, source); // Deep copies from the source to the destination.
	}
